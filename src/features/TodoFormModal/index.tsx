...
const TodoFormModal: React.FC = () => {
  ...
  useEffect(() => {
    // When the modal is opened, check if there's a previous todo for the selected date
    const previousTodo = todoList.find(
      (item) => item.date.toDateString() === selectedDate.toDateString()
    );

    if (previousTodo) {
      setTodo(previousTodo.content);
    } else {
      setTodo(""); // If there's no previous todo, reset the input field
    }
  }, [isOpen, selectedDate, todoList]);
  ...
  const addTodo = useRecoilCallback(
    ({ snapshot, set }) =>
      () => {
        ...
        // Check if there's a previous todo for the selected date
        ...
        if (existingTodoIndex !== -1) {
          ...
          updatedTodoList[existingTodoIndex].content = todo;
